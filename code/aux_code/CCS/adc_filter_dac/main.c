...
//	b2[1] = -1;
	...
	memcpy(bytes_saida, y, sizeof(float)*N);
//	memcpy(bytes_saida, x, sizeof(float)*N);

	while(1){
		for(i=0;i<(N);i++){
			//SysCtlDelay(5000);
			//UARTCharPut(UART0_BASE, bytes_saida[i]);

			value = floor(((y[i]/8000.0) + 0.5)*255);
			saida = (value & 0xFF);
			//y = (pui32ADC0Value[0]>>4);

			GPIOPinWrite(GPIO_PORTB_BASE, (GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7), saida);
